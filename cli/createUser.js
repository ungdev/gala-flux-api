...
Flux.startCLI().then(() => {
    Flux.log.info();
    Flux.log.info();
    Flux.log.info('=========== Create user ===========');
    Flux.log.info('This command will help you to create an user from the commandline.' +
    ...
    .then((answers) => {
        let user = {
            name: answers1.name,
            teamId: teamMap[answers1.teamId],
        };
    ...
    .then((user) => {
        Flux.log.info('Created user:', user.dataValues);
