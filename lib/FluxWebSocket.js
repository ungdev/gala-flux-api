...
class FluxWebSocket {
    ...
    _handleNewSocket(socket) {
        Flux.info('New websocket client !');
    ...
    injectHttpRequest(socket, data) {
        if(!data.method || !data.url) {
            Flux.warn('Socket.io request ignored because method or url is not defined');
            return;
        }
        ...
        if(data.requestId) {
            ...
            let timeout = setTimeout(function () {
                Flux.error('Timeout: No request response after 60 secs for `', data.method, data.url, '`');
