...
class FluxBootstrap {
    ...
    static initWebsocket() {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Configure origins
            let originString = '';
            for (let origin of Flux.config.cors.origin || []) {
                let originUrl = new URL(origin);

                // Socket io need port so we add it
                let port = originUrl.port;
                if(!originUrl.port && originUrl.protocol == 'https:') {
                    port = 443;
                }
                else if(!originUrl.port && originUrl.protocol == 'http:') {
                    port = 80;
                }
                originString += originUrl.hostname + ':' + port + ' ';
            }
            websocket.io.set('origins', originString);
